...
Please analyze the translation quality, and finally give the opinion, put your opinion in <opinion> xml tag."""
    ...
    for data in all_data:
        ...
        for record in translations:
            ...
            if key not in dataset:
                dataset[key] = []
            dataset[key].append(train_record)
    ...
    if output_dir.startswith('s3://'):
        ...
        for filename, records in dataset.items():
            ...
            train_cnt = min(math.ceil(record_cnt * 0.9), 2000)
            test_cnt = math.ceil(train_cnt * 0.2)
            train_obj_key = f"{prefix}trainset/{filename}-{train_cnt}.json"
            test_obj_key = f"{prefix}testset/{filename}-{test_cnt}.json"
            train_data_all.extend(records[:train_cnt])
            
            s3.put_object(
                Bucket=bucket,
                Key=train_obj_key,
                Body=json.dumps(records[:train_cnt], ensure_ascii=False, indent=2).encode('utf-8')
            )
            if record_cnt > train_cnt:
                s3.put_object(
                Bucket=bucket,
                Key=test_obj_key,
                Body=json.dumps(records[train_cnt:(train_cnt+test_cnt)], ensure_ascii=False, indent=2).encode('utf-8')
            )
        
            print(f"\nSummary data has been saved to s3://{bucket}/{train_obj_key} and s3://{bucket}/{test_obj_key}")

        train_data_all_cnt = len(train_data_all)
        train_data_all_obj_key = f"{prefix}trainset/merge-{train_data_all_cnt}.json"
        s3.put_object(
            Bucket=bucket,
            Key=train_data_all_obj_key,
            Body=json.dumps(train_data_all, ensure_ascii=False, indent=2).encode('utf-8')
        )
    else:
