...
def process_file_group(product_category, file_group, source_bucket):
    ...
    for file_key in file_group:
        ...
        for record in data:
            ...
            if score in [4,5]:
                pos_all_data.append(record)
            else:
                neg_all_data.append(record)
...
def build_sft_dataset(trainset, bucket, s3_prefix_dir):
    s3_client = boto3.client('s3')
    for tag, records in trainset.items():
        train_example_list = []
            ...
            try:
                train_example = {
                    "messages": [
                        {
                            "role": "system",
                            "content": f"""You are an expert linguist specializing in translation quality assessment. Your task is to evaluate the translation produced by translation model. 
The key words should be translated accurately with appropriate words. Please evaluate translation quality, and give your rating(0.0-5.0)."""
                        },
                        {
                            "role": "user",
...
{record["translation"]}
                        ...
                        {
                            "role": "assistant",
                            "content": f"<think>{record["thought"]}</think>, my rating is {record["scores"][-1]}"
                        }
                    ]
                }
        ...
        key = f"{s3_prefix_dir}/trainset-{tag}.json"
        s3_client.put_object(
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            ...
            neg_all_data.extend(neg_records)
            neg_record_cnt = len(neg_records)
            random.shuffle(pos_records)
            pos_all_data.extend(pos_records[:neg_record_cnt])
    ...
    neg_cnt = len(neg_all_data)
    pos_cnt = len(pos_all_data)
    train_neg_cnt = int(neg_cnt * args.test_ratio)
    train_pos_cnt = int(pos_cnt * args.test_ratio)

    testset = {
        "neg" : neg_all_data[:train_neg_cnt],
        "pos" : pos_all_data[:train_pos_cnt]
    }
    ...
    for tag, records in trainset.items():
        obj_key = f"{args.output_dir}/trainset/{tag}-{len(records)}.json"
    ...
    for tag, records in testset.items():
        obj_key = f"{args.output_dir}/testset/{tag}-{len(records)}.json"
        logger.info(f"neg_data will save to ${obj_key}")
